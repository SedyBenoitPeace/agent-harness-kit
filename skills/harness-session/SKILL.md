...
3. **Clean worktree:** run the baseline full gate —
   `bash scripts/e2e.sh` (M15-002 adds a concise `run-gate.sh` wrapper;
   until then this skill invokes the target's gate directly so it is
   usable standalone). Confirm green before touching code.
...
8. Run the final full gate, flip only the selected feature's status to
   `passing`, append one `PROGRESS.md` entry, commit explicit paths, then
   STOP — do not start a second feature.
